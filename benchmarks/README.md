...
This suite measures the public `SearchEngine` and `AstEngine` APIs. Timings include Python validation, PyO3 conversion, native work, and result mapping. They exclude fixture generation, proposal setup for apply measurements, provider calls, agent construction, and model loops.
...
`.github/workflows/benchmarks.yml` runs this smoke set for benchmark and native search/AST changes, and supports manual dispatch. It verifies execution only; shared-runner timings are not retained or compared.
...
These are warm-filesystem, steady-state operation timings. Process startup, extension import, fixture construction, and deliberate cold-cache manipulation are outside the contract. Add a separately named cold-start scenario only when application startup becomes a measured product requirement.
